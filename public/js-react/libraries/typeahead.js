...
dependenciesLoader(["React", "ReactDOM"], function(){
    ...
    Typeahead = React.createClass({
        ...
        render: function(){
            ...
            var typeahead = (
                <ul style={styleTypeahead} ref="typeaheadInput" className="typeaheadContainer">
                    {this.state.matchingItems.map(function(item, index){
                        ...
                        if(typeof(item.icon) != "undefined"){
                            img = (<img src={item.icon} style={{width: "25px", marginRight: "5px"}} />);
                        }
                        return (
                            <li style={{textAlign: "left"}} onClick={target.itemSelected.bind(target, item)} className={className} item={item.item} key={index}>{img}{item.string}</li>
                        );
                    })}
                </ul>
            );
            ...
            return (
                <div>
                    <input type={this.props.typeContent}
