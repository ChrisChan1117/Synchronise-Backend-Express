...
dependenciesLoader(["$", "_", "React", "ReactDOM", "Loader", "Promise", "Synchronise", "ModalErrorParse", "KeyEventController", "Mousetrap"], function(){
    DatabaseFunctions = (function(){
        ...
        var SchemaUpdatePopup = React.createClass({
			...
			undo: function(){
				if(this.state.schemaChangeHistory.length &&
				   this.state.positionInHistory>0){
					this.setState({
			...
			redo: function(){
				if(this.state.positionInHistory < this.state.schemaChangeHistory.length-1 &&
				   this.state.schemaChangeHistory.length){
					this.setState({
							schema: this.state.schemaChangeHistory[this.state.positionInHistory+1],
							positionInHistory: this.state.positionInHistory+1
						});
            ...
            render: function(){
				...
				if(this.state.loading){
					loader = (
						<div className="row-fluid" style={{textAlign: "center"}}><Loader/></div>
					);
				}else{
					...
					undoRedoButtons = (
						<div className="btn-group" style={{position: "absolute", left: "10px"}}>
							<button className={"btn btn-default btn-sm "+undoStatus} onClick={this.undo}><i className="fa fa-undo"></i></button>
						    <button className={"btn btn-default btn-sm "+redoStatus} onClick={this.redo}><i className="fa fa-repeat"></i></button>
						</div>
					);

					footer = (
						<div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 footer">
                            <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6" align="center">
                                <button className="btn btn-default" onClick={this.cancel}>{this.props.cancelButton}</button>
                            </div>

                            <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6" align="center">
                                <button className="btn btn-primary" onClick={this.confirm}>{this.props.confirmButton}</button>
                            </div>
                        </div>
					);
				}
                ...
                return (
                    <div id="databaseSchemaUpdatePopup" className={this.state.className} style={this.state.styleForContainer}>
						<div id="databaseSchemaUpdateSubpopup"></div>

                        <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 title" align="center">
							{undoRedoButtons}
                        ...
                        <div className="row-fluid">
							<div className="col-xs-12" id="contentSIO" style={this.state.styleForContent}>
								{this.state.schema.schema.map(function(item){
									return <TableItem status={item.status}
											          title={item.title}
													  fields={item.fields}
        ...
        var ProgressUpdate = React.createClass({
            ...
            complete: function(){ // Set progress has complete
                this.setProgress(1);
            },
            reset: function(){ // Set progress to beginning
                this.setProgress(0);
            },
            render: function(){
                return (
                    <div className="progressUpdate" style={this.state.styleToApply}></div>
                );
            }
        });
            ...
            render: function(){
                ...
                return (
                    <div className={this.state.className}>
                        <div className="row-fluid">
                            <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 title" align="center">
                                <p>{this.props.popupTitle}</p>
                            </div>

                            <div className="content">
                                <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6 SIOSide">
                                    <legend className="col-xs-12">{this.props.titleBlockLeft}</legend>

                                    <div className="col-xs-12">
                                        <div className="panel panel-default">
                                            <div className="panel-heading">
                                                <h3 className="panel-title">{this.props.elementTitleBlockLeft}</h3>
                                            </div>
                                        </div>
                                    </div>
                                </div>

                                <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6 ThirdSide">
                                    <legend className="col-xs-12">{this.props.titleBlockRight}</legend>
                                    <ul className="list-unstyled">
                                        {this.props.blockRightElements.map(function(item, index){
                                            return <SchemaUpdateSubpopupItem value={item.value}
																			 title={item.title}
                            ...
                            <div className="col-lg-12 col-md-12 col-sm-12 col-xs-12 footer">
                                <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6" align="center">
                                    <button className="btn btn-default cancel" onClick={this.cancel}>Cancel</button>
                                </div>
                                <div className="col-lg-6 col-md-6 col-sm-6 col-xs-6" align="center">
                                    <button className="btn btn-primary rename" onClick={this.confirm}>Rename</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <li>
                        <div className="radio">
                            <label><input type="radio"
										  ...
										  value={this.props.value} />{this.props.title}</label>
                        </div>
                    </li>
                );
            }
        ...
        var TableItem = React.createClass({
			...
			renameTable: function(){
				...
				if(this.state.status == "remove"){
					...
					ReactDOM.render(<SchemaUpdateSubpopup popupTitle="Click on the element on the table that corresponds to the new name of the table on the left"
            ...
            render: function(){
				...
				if(this.state.status == "remove"){
					renameTableBlock = (
						<a href="#" onClick={this.renameTable}>Has this table been renamed ?</a>
					);
				}

                return (
                    <div className={this.state.defaultStyle + " " + this.state.status + " " + this.state.customStyle}>
                        <div className="panel panel-default">
                            <div className="panel-heading">
                            ...
                            <div className="panel-body">
                                <ul className="list-unstyled">
                                    {this.props.fields.map(function(item){
                                        return <TableItemField name={item.name}
												               status={item.status}
        ...
        var TableItemField = React.createClass({
			renameField: function(){
				...
				ReactDOM.render(<SchemaUpdateSubpopup popupTitle="Click on the field on the right that corresponds to the new name of the field on the left"
            ...
            render: function(){
				var renameBlock = "";
				if(this.props.tableStatus == "normal" &&
			       this.props.status == "remove"){
					renameBlock = (<span><span>&nbsp;</span><a href="#" onClick={this.renameField} style={{float: "right"}}>renamed?</a></span>);
				}

				var sign = "";
				if(this.props.status == "add"){
					sign = (<span><span style={{color: "green"}}>+</span><span>&nbsp;</span></span>);
				}

				if(this.props.status == "remove"){
					sign = (<span><span style={{color: "red"}}>-</span><span>&nbsp;</span></span>);
				}

                return (
                    <li className={this.props.status}>{sign}{this.props.name}{renameBlock}</li>
                );
            }
        });
