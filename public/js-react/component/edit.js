    ...
    var Component = React.createClass({
        ...
        render: function(){
            ...
            if(this.state.loading){
                contentForLeftBar = <Loader/>;
                contentForRightBar = <Loader/>;
            }else{
                contentForLeftBar = (
                    <LeftBar  component={this.state.component}
                ...
                contentForRightBar = (
                    <RightBar component={this.state.component}
                              loading={this.state.loading}/>
                );
            }
            ...
            if(this.state.component.is_forked){ //// VIEW MODE
                leftBar = (
                    <div className="col-lg-9 col-md-9 col-sm-12 col-xs-12">{contentForLeftBar}</div>
                );
            }else{ // EDIT MODE
                leftBar = (
                    <div className="col-lg-9 col-xs-12">{contentForLeftBar}</div>
                );
            ...
            if(this.state.component.published == true){
                ...
                (function() { var d = document, s = d.createElement('script'); s.src = '//synchronise.disqus.com/embed.js'; s.setAttribute('data-timestamp', +new Date()); (d.head || d.body).appendChild(s); })();

                var titleCommentBox = "";
                if(this.state.component.user_id != Synchronise.User.current().id){ // Not owner
            ...
            if(this.state.component.is_forked){ // VIEW MODE
                content = (
                    <div className="row">
                        <div className={classForRightBar}>{contentForRightBar}</div>
                        {leftBar}
                    </div>
                );
            }else if(this.state.component.user_id != Synchronise.User.current().id && this.state.loading == false){ // VIEW FROM SHARED PROJECT
                content = (
                    <div>
                        <div className="row">
                            <div className="col-xs-12"><div className="alert alert-info" style={{textAlign:"center"}}>You are in view mode only. The purpose of this page, is to help you learn how other developers are building components. The changes you make on this interface will no be saved.</div></div>
                            {leftBar}
                            <div className={classForRightBar}>{contentForRightBar}</div>
                        </div>
                        {commentBox}
