...
dependenciesLoader(["React", "ReactDOM", "_", "Loader"], function(){
    ...
    ComponentCatalog = React.createClass({
        ...
        render: function(){
            ...
            if(this.state.loading){
                content = (<div style={{textAlign: "center"}}><Loader/></div>);
            }else if(this.state.projects.length){
                content = (
                    <div className="col-xs-12">
                        <div className="searchContainer">
                            <input type="search" className="form-control" placeholder="Search by name" onChange={this.searchFieldChanged}/>
                            <i className="fa fa-search" style={{position: "absolute", right: "15px", marginTop: "-25px"}}></i>
                        </div>

                        <div className="listOfProjects row">
                            <Transition>
                            {this.state.projects.map(function(row){
                                return <ComponentCatalogProject id_project={row.id}
                ...
                content = (
                    <div className="col-xs-12">
                        ...
                        <div className="row-fluid" style={{textAlign: "center", padding: "5px", margin: "5px", paddingTop: "40px"}}>No results, try a different name.</div>
                    </div>
                );
            }else{
                content = (
                    <div className="col-xs-12">
                        ...
                        <div className="row-fluid" style={{textAlign: "center", padding: "5px", margin: "5px", paddingTop: "40px"}}>No components available. <br/><a href={"/component?displayModalCreate=true&backuri=" + encodeURIComponent("/workflow/edit?id=" + urlH.getParam("id")) + "&backlabel=" + encodeURIComponent("Back to Workflow")} className="btn btn-primary btn-xs">Create a new component</a></div>
                    </div>
                );
            }
            ...
            var forking = (
                <div id="forkingMask" style={{display: displayForForking, position: "fixed", background: "rgba(255,255,255,0.95)", marginTop: "-10px", marginLeft: "-10px", zIndex: 11, fontWeight: "bold", fontFamily: "monospace", fontSize: "2em", textAlign: "center"}}><Loader/></div>
            );

            return (
                <div className="card componentCatalog"
                     data-toggle="tooltip"
    ...
    var ComponentCatalogProject = React.createClass({
        ...
        render: function(){
            ...
            return (
                <div className={"col-lg-6 col-sm-6 col-xs-12 project " + this.state.classForProject} style={{paddingLeft: "5px", paddingRight: "5px"}}>
                    {loader}
                    <div className="card title" onClick={this.toggleListOfComponents} style={{background: this.state.bg_color}}>
                        <div className="content">
                            <img className="logo" src={this.state.icon} style={{filter: this.state.icon_flts, borderRadius: "5px"}}/>
                            <span style={{color: this.state.txt_color}}>{this.state.name}</span>
                        </div>
                    </div>

                    <ComponentCatalogProjectContent opened={this.state.listOfComponentsOpened}
    ...
    var ComponentCatalogProjectContent = React.createClass({
        ...
        render: function(){
            ...
            if(this.props.opened){
                ...
                content = (
                    <div className={"card listOfComponents " + classForListOfComponents} style={{border: "1px solid " + this.props.bg_color}}>
                        {this.state.components.map(function(row, index){
                            return <ComponentCatalogProjectComponent data={row}
                                                                     clickOnComponent={target.props.clickOnComponent}
        ...
        render: function(){
            return (
                <div className="component">
                    <div className="addToWorkflowContainer toDisplayOnHover">
                        <button className="button" onClick={this.addToWorkflow}>Add to workflow</button>
                    </div>

                    <b className="toHideOnHover">{this.props.data.name}</b><br/>
                    <small className="toHideOnHover">{this.props.data.description}</small>
                </div>
            );
        }
    });
